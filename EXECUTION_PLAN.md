...
| M7 - IC Chair Triage | Implemented | Verified | Built from `docs/milestones/m7_spec.md`: non-persistent Idea Triage, deterministic triage helper/tests, Agenda `Investigate idea`, explicit case/watchlist transitions, case-stage labels, verification-first case-file copy, and committee-review gating during draft intake. `npm test`, `npm run lint`, `npm run build`, and isolated `node scripts/run.js qa m7` passed on 2026-06-18 with retained evidence at `issues/qa/2026-06-18T13-53-23-771Z/report.json`. Browser QA used the fallback Edge/CDP harness with updated Agenda / Idea Triage selectors after creation actions moved out of the Library rail. |
