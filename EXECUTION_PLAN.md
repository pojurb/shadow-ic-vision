...
| M7 - IC Chair Triage + Everyday-User Front Door | Implemented | Verified | Built from `docs/milestones/m7_spec.md`: non-persistent Idea Triage, deterministic triage helper/tests, plain-language `Explore an idea` / `Start review` / `Save to watchlist` workflow, persistent temporary-vs-saved cues, prompt-only Evidence Locker carry-forward on `Start review`, `Check the facts` review framing, startup/private routing copy cleanup, and deterministic saved-review recovery actions. `npm test`, `npm run lint`, `npm run build`, and isolated `node scripts/run.js qa m7` passed on 2026-06-20 with retained evidence at `issues/qa/2026-06-20T09-59-50-458Z/report.json`. Browser QA used the fallback Edge/CDP harness. |
