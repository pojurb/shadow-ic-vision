...
| M2 - Manual Asset Entry | Implemented | Verification partial | Built from `docs/milestones/m2_spec.md`: manual `valuationMode`, nullable engine fields, manual metadata/risk prompts, dual `+ NEW ANALYSIS` / `+ MANUAL ASSET` entry points, manual thesis/detail editing, backup/snapshot compatibility, and portfolio-picker exclusion. `npm test`, `npm run lint`, and `npm run build` passed on 2026-06-16. Manual browser-flow verification is still pending because the local in-app browser tool crashed during setup in this environment. |
