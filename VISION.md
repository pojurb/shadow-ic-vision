...
The result is frustration: poorly timed entries, late exits, and missed opportunities on high-conviction ideas simply because the investor lost track of the changing facts.

## 3. The Product Promise

**Codex Protocol is an AI-assisted Investment Committee that tracks your theses, challenges your assumptions with verified evidence, and turns scattered conviction into disciplined decisions.**
...
3. **Insights:** What verified data (filings, transcripts) has emerged this week that is worth reading?
...
1. **Supporting Evidence:** The exact filing, transcript, or data point that triggered the concern, properly cited.
...
1. **Never present unsupported information as verified:** Every quantitative claim must be cited from a trusted source. If the data is missing, the system admits it.
2. **Show uncertainty and source freshness:** The system must clearly indicate when data is stale or when an inference is probabilistic rather than factual.
3. **Ensure user sovereignty:** The system must fully support data correction, deletion, long-term retention, and export.
4. **Never silently train:** The system will never silently train models or recalibrate algorithms using private user data without explicit consent.

## 7. Product Scope and Sequencing
...
- Focus: Core thesis intake, manual or simple scheduled monitoring, structured IC briefing generation, and a clear decision ledger.
...
## 8. Measurable Success (Product Metrics)
...
1. **Weekly Briefing Usefulness:** How consistently the user reads and acts upon the Sunday briefing.
2. **Citation Accuracy:** The percentage of important claims in the briefing that are properly cited and verified.
3. **Assumption Changes Detected:** The volume of relevant shifts in underlying data successfully surfaced to the user.
4. **Thesis-Review Completion:** How often the user completes the loop from warning to documented decision.
5. **Trust and Correction Rates:** How often the user overrides or corrects the system (a lower rate indicates higher trust and accuracy).
6. **Time to Decision:** The reduction in time required for the user to reach a well-documented, evidence-based decision compared to their old process.
